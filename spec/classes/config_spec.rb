  ...
  on_supported_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      context 'with default values' do
        ...
        if os_facts[:os][:release][:minor] && os_facts[:os][:release][:major].to_i == 8 && os_facts[:os][:release][:minor].to_i < 4
      ...
      context 'with non-default parameters impacting /etc/pam.d/sudo*' do
        ...
        context 'with enable_ssh_agent_auth = true and a custom authorized_keys_command' do
          let(:params) do
            {
              enable_ssh_agent_auth: true,
              ssh_agent_auth_authorized_keys_command: '/usr/local/bin/my_keys_helper',
            }
          end

          it {
            is_expected.to contain_file('/etc/pam.d/sudo')
              .with_content(%r{^auth sufficient pam_ssh_agent_auth\.so authorized_keys_command=/usr/local/bin/my_keys_helper$})
          }
        ...
        context 'with enable_ssh_agent_auth defaulting to false' do
          it { is_expected.not_to contain_file('/etc/pam.d/sudo').with_content(%r{pam_ssh_agent_auth\.so}) }
          it { is_expected.not_to contain_file('/etc/pam.d/sudo-i').with_content(%r{pam_ssh_agent_auth\.so}) }
        end
